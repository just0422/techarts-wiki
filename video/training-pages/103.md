...
The camera will actually zoom at different speeds based off of how hard the handle is twisted. This is useful for many parts of service because our team tends to zoom at different speeds (depending on the section of service). There is one last knob on the handle called the zoom control. Located at the center, it actually adjusts the maximum speed of the zoom on our handle. Rotate it clockwise and the speed of our zoom is faster. The opposite happens when we rotate the knob counter clockwise.
...
**INSERT HANDLE ZOOM**
# Focus
Let's not leave the area of the zoom ring just yet. The ring in front of the camera controls focus. There are 2 settings we can use for focus: automatic and manual. When set to manual, we can adjust the focus ring to modify the sharpness of the shot. We can't establish an absolute rule about which way to twist the focus ring to get a sharp image, because it varies for different zoom distances and camera distances from the subject. The best practice is to take the image out of focus (in either direction) and slowly bring it in focus until it's perfect.
